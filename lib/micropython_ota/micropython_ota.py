...
def check_version(host, project, auth=None, timeout=5) -> (bool, str):
    try:
        if 'version' in uos.listdir():
            ...
            current_version = ''
        print(f"Current version on board: {current_version}")

        ota_url = f'{host}/{project}/version'
        print("Checking required version from remote OTA server: {ota_url} ...")
        if auth:
            print("Using HTTP basic authentication")
            response = urequests.get(ota_url, headers={'Authorization': f'Basic {auth}'}, timeout=timeout)
        else:
            print("Not using HTTP auth")
            response = urequests.get(ota_url, timeout=timeout)
